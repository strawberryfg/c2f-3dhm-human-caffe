...
namespace caffe {
	...
	void DeepHumanModelH36MGenPredMono3DLayer<Dtype>::LayerSetUp(
		const vector<Blob<Dtype>*>& bottom, const vector<Blob<Dtype>*>& top) {
		joint_num_ = this->layer_param_.gen_pred_mono_3d_param().joint_num();
		camera_parameters_prefix_ = this->layer_param_.gen_pred_mono_3d_param().camera_parameters_prefix();

	}
	template <typename Dtype>
	void DeepHumanModelH36MGenPredMono3DLayer<Dtype>::Reshape(
		const vector<Blob<Dtype>*>& bottom, const vector<Blob<Dtype>*>& top) {
	...
	template <typename Dtype>
	void DeepHumanModelH36MGenPredMono3DLayer<Dtype>::Forward_cpu(const vector<Blob<Dtype>*>& bottom,
		const vector<Blob<Dtype>*>& top) {
		int batSize = (bottom[0]->shape())[0];
		const Dtype* pred_2d_data = bottom[0]->cpu_data();    //predicted 2d in cropped bounding box [0, 1]
		const Dtype* pred_z_data = bottom[1]->cpu_data();     // predicted depth(in camera frame)
		...
		for (int t = 0; t < batSize; t++) {
			int Pid = t * joint_num_ * 2, Zid = t * joint_num_;
			...
			{
				double proj_u = pred_2d_data[Pid + i * 2], proj_v = pred_2d_data[Pid + i * 2 + 1]; // in the range of [0, 1]
				...
				double pred_z = pred_z_data[Zid + i];
				double pred_x = (proj_u - c[0]) / f[0] * pred_z;
				double pred_y = (proj_v - c[1]) / f[1] * pred_z;
				top_data[Tid + i * 3] = pred_x;
				top_data[Tid + i * 3 + 1] = pred_y;
				top_data[Tid + i * 3 + 2] = pred_z;
			}
		}
	}

	template <typename Dtype>
	void DeepHumanModelH36MGenPredMono3DLayer<Dtype>::Backward_cpu(const vector<Blob<Dtype>*>& top,
		const vector<bool>& propagate_down, const vector<Blob<Dtype>*>& bottom) {
	
	}

#ifdef CPU_ONLY
	STUB_GPU(DeepHumanModelH36MGenPredMono3DLayer);
#endif

	INSTANTIATE_CLASS(DeepHumanModelH36MGenPredMono3DLayer);
	REGISTER_LAYER_CLASS(DeepHumanModelH36MGenPredMono3D);
}